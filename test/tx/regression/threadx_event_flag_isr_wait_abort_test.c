...
/* The window this test waits for is probabilistic, and the handler above says as
   ...
   load or instrumentation, which is exactly when the wait gets long.

   The count asked for is left at forty. Unlike the wait abort test, this loop
   reaches forty windows in well under two seconds in all five configurations, so
   there is nothing to gain by asking for fewer and the budget is never
   approached by a healthy run.  */
