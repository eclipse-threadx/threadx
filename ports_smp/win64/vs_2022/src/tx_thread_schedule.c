...
DWORD       wait_status;
    ...
    while (1)
    {
            ...
            if (_tx_thread_current_ptr[core] == TX_NULL)
            {
                ...
                if ((execute_thread != TX_NULL) && (execute_thread -> tx_thread_smp_core_control != 0U))
                {
                    ...
                    else if (execute_thread -> tx_thread_win32_suspension_type == 2U)
                    {
                        ...
                        wait_status =  WaitForSingleObject(execute_thread -> tx_thread_win32_thread_start_semaphore, INFINITE);
        ...
        (void) WaitForSingleObject(_tx_win32_scheduler_event, INFINITE);
    }
}
