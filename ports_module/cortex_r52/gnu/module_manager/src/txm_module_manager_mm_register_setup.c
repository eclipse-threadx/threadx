...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _txm_module_manager_mm_register_setup              Cortex-R52       */
/*                                                           6.1.6        */
/*  AUTHOR                                                                */
/*                                                                        */
...
/*  DESCRIPTION                                                           */
/*                                                                        */
/*    This function sets up the Cortex-R52 MPU register definitions based */
...
/*                                                                        */
/**************************************************************************/
/* Two departures from the Armv8-M version this was derived from, both
   deliberate.

   The address mask is 64-byte, not 32-byte.  PRBAR and PRLAR hold attributes in
   the bits below the granule, so masking to the wrong boundary does not fault --
   it silently writes address bits into the shareability and permission fields
   and the region comes up with attributes nobody asked for.

   Regions are non-shareable rather than inner-shareable.  The M33 port marks
   module memory inner-shareable; the RTU here is a single Cortex-R52 and every
   region in the board support package's map is non-shareable, so matching it
   keeps one memory model across the kernel and its modules.  A multi-core RTU
...
ULONG   callback_stack_size;
    ...
    module_instance -> txm_module_instance_mpu_registers[TXM_MODULE_MPU_DATA_INDEX].txm_module_mpu_region_base_address = ((ULONG) module_instance -> txm_module_instance_data_start & TXM_MODULE_MPU_ADDRESS_MASK) | TXM_MODULE_ATTRIBUTE_NON_SHAREABLE | TXM_MODULE_ATTRIBUTE_READ_WRITE | TXM_MODULE_ATTRIBUTE_EXECUTE_NEVER;
    ...
    module_instance -> txm_module_instance_mpu_registers[TXM_MODULE_MPU_DATA_INDEX].txm_module_mpu_region_limit_address = (((ULONG) module_instance -> txm_module_instance_data_start + data_size - 1) & TXM_MODULE_MPU_ADDRESS_MASK) | TXM_MODULE_ATTRIBUTE_INDEX | TXM_MODULE_ATTRIBUTE_REGION_ENABLE;
