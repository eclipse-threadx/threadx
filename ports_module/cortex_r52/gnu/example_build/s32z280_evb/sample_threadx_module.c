...
/* The shared status word, at the base of the module area.
   ...
   S32Z_MODULE_STATUS_BASE in platform.h and ORIGIN(MODULE) in link_module.lds;
...
#define MODULE_STATUS_ADDRESS           0x317F0000UL
...
/* An address the module has no business touching, used by step 3.
   ...
   The base of DRAM0, where the manager's own data and stacks live on this
   board: that region is EL1 read/write and execute-never, so a User-mode read
   of it is a permission fault and a User-mode instruction fetch from it is one
   too.  Both are what step 3 needs, and both prove the KERNEL'S memory is what
   a module cannot reach -- which is the property being demonstrated, rather
   than the absence of a mapping.
