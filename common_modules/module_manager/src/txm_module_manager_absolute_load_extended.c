...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _txm_module_manager_absolute_load_extended          PORTABLE C      */
/*                                                           6.5.2        */
/*  AUTHOR                                                                */
/*                                                                        */
...
/*    execution.                                                          */
/*                                                                        */
/*    An absolutely located module has its code and its data placed at    */
/*    fixed addresses by the module's linker script. The module preamble  */
/*    carries only the code and data sizes, so the module manager cannot  */
/*    derive the module's data location on its own. This service accepts  */
/*    that address from the caller, which allows the module instance's    */
/*    data area to be described correctly.                                */
/*                                                                        */
/*  INPUT                                                                 */
/*                                                                        */
...
/*    module_data_location              Module data location, or TX_NULL  */
/*                                        when the data location is not   */
/*                                        known to the caller. Memory     */
/*                                        protection cannot be used when  */
/*                                        it is TX_NULL.                  */
/*                                                                        */
/*  OUTPUT                                                                */
/*                                                                        */
...
/*                                                                        */
/*    Application code                                                    */
/*    _txm_module_manager_absolute_load Deprecated wrapper                */
/*                                                                        */
/**************************************************************************/
UINT  _txm_module_manager_absolute_load_extended(TXM_MODULE_INSTANCE *module_instance, CHAR *module_name, VOID *module_location, VOID *module_data_location)
{
...
CHAR                    *memory_ptr;
    ...
    if (/* Ensure the requested properties are supported.  */
        ...
        /* Memory protection requires a known data area, since the memory protection hardware must
           be programmed to cover it.  */
        ((module_properties & TXM_MODULE_MEMORY_PROTECTION) && (module_data_location == TX_NULL))
        )
    {
    ...
    /* Since this is an absolute load, check the alignment of the module's data area when the caller
       supplied its location.  */
    if (module_data_location != TX_NULL)
    {
        if ((((ALIGN_TYPE) module_data_location) & (((ALIGN_TYPE) data_alignment) - ((ALIGN_TYPE) 1))) != ((ALIGN_TYPE) 0))
        {

            /* Release the protection mutex.  */
            _tx_mutex_put(&_txm_module_manager_mutex);

            /* Invalid alignment, return error.  */
    ...
    /* Compute the beginning and end of the data area. An absolutely located module has its data
       placed at a fixed address by its linker script, and that address is not carried in the
       module preamble. It must therefore be supplied by the caller.  */
    if (module_data_location != TX_NULL)
    {

        module_instance -> txm_module_instance_data_start =  module_data_location;

        /* A zero data size is valid, so guard against wrapping when computing the end address.  */
        if (data_size != 0)
        {
            module_instance -> txm_module_instance_data_end =  (VOID *) (((CHAR *) module_data_location) + (data_size - 1));
        }
        else
        {
            module_instance -> txm_module_instance_data_end =  module_data_location;
        }
    ...
    else
    {
        ...
        /* The caller does not know where the module's data area is, so the module instance cannot
           describe it. Report an empty data area rather than an invalid one.  */
        module_instance -> txm_module_instance_data_start =  TX_NULL;
        module_instance -> txm_module_instance_data_end =    TX_NULL;

        /* Save the size of the data area.  */
        module_instance -> txm_module_instance_data_size =  0;
    }
    ...
    /* Setup the start of the module data section. For an absolute load, the byte pool allocation
       holds only the two default thread stacks. The module's data section is the absolutely
       located area supplied by the caller.  */
    module_instance -> txm_module_instance_module_data_base_address =  module_data_location;
