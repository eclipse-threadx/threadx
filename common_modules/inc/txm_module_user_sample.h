...
/* TXM_MODULE_OBJECT_DEALLOC_STRICT
   When this symbol is defined, _txm_module_object_deallocate() returns
   TX_DELETE_ERROR whenever it detects that the memory being released still
   backs a live ThreadX kernel object (timer, thread, semaphore, mutex, queue,
   event flags group, block pool, or byte pool).  The function always performs
   the necessary kernel cleanup (equivalent to calling tx_*_delete()) before
   releasing the pool memory, so the system is left in a safe state regardless
   of this setting.

   Without this define (the default) the cleanup is silent and TX_SUCCESS is
   returned, preserving backward compatibility with existing code that does not
   check for this condition.

   Recommended for new development and for applications that run untrusted
   modules, so that deallocation of live objects is caught early. */

/* #define TXM_MODULE_OBJECT_DEALLOC_STRICT */
