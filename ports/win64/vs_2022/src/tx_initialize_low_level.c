...
static HANDLE                   _tx_win32_timer_thread_handle;
static HANDLE                   _tx_win32_timer_start_event;
...
void _tx_initialize_start_interrupts(void)
{
    ...
    /* Create the event that starts the periodic timer thread.  */
    _tx_win32_timer_start_event =  CreateEvent(NULL, TRUE, FALSE, NULL);
    if (_tx_win32_timer_start_event == NULL)
    {
        printf("ThreadX Win64 error creating timer start event!\n");
        while (1)
        {
        }
    }
    ...
    /* Start the waitable timer and then release the timer thread.  */
    due_time.QuadPart =  -(((LONGLONG) TX_TIMER_PERIODIC) * 10000LL);
    if (SetWaitableTimer(_tx_win32_timer_handle, &due_time, (LONG) TX_TIMER_PERIODIC, NULL, NULL, FALSE) == 0)
    {
    ...
    SetEvent(_tx_win32_timer_start_event);
}
...
static DWORD WINAPI _tx_win32_timer_thread_entry(LPVOID thread_input)
{
    ...
    /* Wait until the kernel enables interrupts.  */
    WaitForSingleObject(_tx_win32_timer_start_event, INFINITE);
