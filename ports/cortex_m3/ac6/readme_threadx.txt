...
MPS2_Cortex_M3 Bare Metal simulator.
...
in the Project Explorer window, right-click on the 'cortex-m3_tx.launch' file, click
'Debug As', and then click 'cortex-m3_tx' from the submenu. This will cause the
...
The entry point in ThreadX for the Cortex-M3 using AC6 tools uses the standard AC6 
Cortex-M3 reset sequence. From the reset vector the C runtime will be initialized.
...
The Cortex-M3 vectors start at the label __tx_vectors or similar. The application may modify
...
Note: the Cortex-M3 requires exception handlers to be thumb labels, this implies bit 0 set.
