...
/* Define ARC HS extension for assigning a hardware register bank to a thread. Note that this API can only be
   called after a thread is created from initialization. It is assumed that interrupts are disabled and the
   initialization code is running in hardware register bank 0. It is also assumed that the application provides
   a vaild register bank number to the API.  */

#ifndef TX_SOURCE_CODE
#define tx_initialize_fast_interrupt_setup      _tx_initialize_fast_interrupt_setup
#define tx_thread_register_bank_assign          _tx_thread_register_bank_assign
#endif

VOID  tx_initialize_fast_interrupt_setup(VOID *stack_ptr);
VOID  tx_thread_register_bank_assign(VOID *thread_ptr, UINT register_bank);
...
CHAR                            _tx_version_id[] = 
                                    "Copyright (c) Microsoft Corporation. All rights reserved.  *  ThreadX ARC_HS/MetaWare Version 6.0 *";
#else
#ifdef TX_MISRA_ENABLE
